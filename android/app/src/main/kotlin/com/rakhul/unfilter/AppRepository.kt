    ...
    private fun generateErrorReport(
        ...
        when {
            currentFreeMemory < 50 -> {
                deviceInfo.append("  CRITICAL: Very low memory (${currentFreeMemory}MB)\n")
            ...
            timeoutCount > filteredTotal / 2 -> {
                deviceInfo.append("  CRITICAL: High timeout rate ($timeoutCount/$filteredTotal)\n")
            ...
            exceptionCount > filteredTotal / 2 -> {
                deviceInfo.append("  CRITICAL: High exception rate ($exceptionCount/$filteredTotal)\n")
                deviceInfo.append("  -> Unexpected errors during processing\n")
                deviceInfo.append("  -> Check logcat for details\n")
            }
            else -> {
                deviceInfo.append("  All tasks failed - unknown cause\n")
